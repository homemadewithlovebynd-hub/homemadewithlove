...
let db = null;
try {
  if (cfg.SUPABASE_URL && cfg.SUPABASE_ANON_KEY && !cfg.SUPABASE_URL.includes("PASTE_YOUR")) {
    db = createClient(cfg.SUPABASE_URL, cfg.SUPABASE_ANON_KEY);
  }
} catch (e) {
...
async function loadProducts(){
  ...
  if(!db){ statusEl.textContent="Products are unavailable until your existing config.js is restored."; return; }
...
document.getElementById("checkout-whatsapp").onclick=async()=>{
  if(!db){ alert("Please restore your working Supabase config.js first."); return; }
...
document.getElementById("auth-button").onclick=async()=>{
  if(!db){ openAuth(); setAuthPanel("login"); document.getElementById("customer-login-message").textContent="Please restore your working Supabase config.js first."; return; }const {data:{session}}=await db.auth.getSession();if(session){document.getElementById("account-name").textContent=`Hello, ${session.user.user_metadata?.full_name||"there"}!`;document.getElementById("account-email").textContent=session.user.email||"";setAuthPanel("account");}else setAuthPanel("login");openAuth();};
...
document.getElementById("logout-customer").onclick=async()=>{if(!db)return; await db.auth.signOut();closeAuth();updateAuthButton();};
async function updateAuthButton(){if(!db){document.getElementById("auth-button").textContent="My Account";return;} const {data:{session}}=await db.auth.getSession();document.getElementById("auth-button").textContent=session?"♙ My Account":"♙ Login / Sign up";} if(db) db.auth.onAuthStateChange(()=>updateAuthButton());
...
function startHeroTimer(){clearInterval(heroTimer);heroTimer=setInterval(()=>showHeroSlide(heroIndex+1),6000);}
...
heroDots.forEach(dot=>dot.addEventListener("click",()=>{showHeroSlide(Number(dot.dataset.slide));startHeroTimer();}));
showHeroSlide(0);startHeroTimer();
